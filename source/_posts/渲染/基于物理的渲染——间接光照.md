...
diffuse部分的法线半球的各个方向的环境光对diffuse都有贡献，而对specular而言，只有在环境光线方向在视线方向的反射方向的附近的光，才能够被人眼看到。也就是说通过$h$来求入射光线的方向$L$。上面已经说过，环境光照没有一个确定的$h$。但是specular brdf是基于$h$的，所以我们必须将$h$求出来。下面我们详细的分析微平面模型以求出$h$。
#### 法线分布函数
...
$$
讲我们采用的Trowbridge-Reitz GGX发现分布函数带入，得
$$
