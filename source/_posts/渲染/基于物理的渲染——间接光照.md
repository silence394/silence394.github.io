...
$$
A' = \int_{\Omega} {A} \cdot D(\omega_h)d\omega_h\ge A
$$

$$
\int D(h)d\omega \ge 1 
$$

$$
\int\limits_{\Omega _x } {A \cdot D\left({{\bf{\omega}}_h } \right)\left({{\bf{v}} \cdot {\bf{h}}} \right){\rm{d}}{\bf{\omega}}_h }  = A\left({{\bf{v}} \cdot {\bf{n}}} \right)
$$

$$
v\cdot n =\int D(h) (v\cdot h) d\omega
$$

v == n
$$
\int D(h) (n \cdot h) d\omega =  \int D(h) \cos\theta d\omega = 1
$$
其中$\theta$为$h$和n的夹角。

$$
p_h(\omega) = D(h)\cos\theta
$$
通常用球面坐标系采样，所以将立体角下的$pdf$转为球面坐标系下。
$$
p_h(\theta, \phi) = \dfrac{\alpha^2cos\theta sin\theta}{\pi((\alpha^2-1) cos^2\theta+1)^2}
$$
由于$p_h(\theta, \phi)$没有$\phi$，很容易得到$p_h(\theta)$：
$$
p_h(\theta)=\int_{0}^{2\pi}p_h(\theta,\phi)d\phi = \dfrac{2\alpha^2cos\theta sin\theta}{((\alpha^2-1) cos^2\theta+1)^2} 
$$
再求$p(\phi|\theta)$：
$$
p_h(\phi|\theta) = \frac{p_h(\theta, \phi)}{p_h(\theta)} = \frac{1}{2\pi}
$$
对$p_h(\theta)$积分，得到其概率分布函数：
$$
...
我们还有随机数$\xi_0$和$\xi_1$没有解决。传统的抽样方式是采用电脑的伪随机数来进行的，这种随机的特征导致每个随机数并不知道其他随机数的任何信息，所以其分布可能会出现丛聚，减慢了收敛速度。但如果使用某些方法使随机数均匀地分布，那么蒙特卡洛方法的模拟过程不会受到影响，并且收敛速度可以加快。

产生较均匀分布的随机数生成方法叫低差异序列（Low Discrepancy Sequence），而我们使用的是其中的一种——Hammersley。其和伪随机数生成的点集对比如下图：
![](http://ww1.sinaimg.cn/mw690/c5c3a364ly1fyxsp8mf8mj20jg0a8aj5.jpg)
有兴趣了解原理的可以阅读[这篇文章](http://holger.dammertz.org/stuff/notes_HammersleyOnHemisphere.html)，在这里我们直接给出代码。
``` glsl
...
```
到这里随机数$\xi_0$和$\xi_1$的生成也搞定了。综合以上可以方便地得出$h$，然后求解渲染方程。
